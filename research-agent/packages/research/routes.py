...
def register(app):
    ...
    def youtube_subs():
        ...
        last_err = None
        # Phase 1: Piped mirrors с exponential backoff
        for inst in cfg.PIPED_INSTANCES:
            backoffs = [0, 1, 2, 4, 8]
            for sleep_s in backoffs:
                if sleep_s:
                    time.sleep(sleep_s)
                try:
                    j = requests.get(f'{inst}/streams/{video_id}', timeout=20,
                                     headers={'User-Agent': 'Mozilla/5.0'}).json()
                except Exception as e:
                    last_err = f'{inst}: {e}'
                    continue
                if not j or 'error' in j:
                    last_err = f'{inst}: {j.get("error", "empty")}'
                    continue
                subs = j.get('subtitles', [])
                if not subs:
                    last_err = f'{inst}: no subtitles'
                    break
                # выбрать лучший: prefer manual → prefer auto → любой auto
                chosen = None
                for s in subs:
                    if s.get('code', '').lower().startswith(prefer_lang) and not s.get('autoGenerated'):
                        chosen = s; break
                if not chosen:
                    for s in subs:
                        if s.get('code', '').lower().startswith(prefer_lang):
                            chosen = s; break
                if not chosen:
                    for s in subs:
                        if s.get('autoGenerated'):
                            chosen = s; break
                if not chosen and subs:
                    chosen = subs[0]
                try:
                    sub_r = requests.get(chosen['url'], timeout=30,
                                         headers={'User-Agent': 'Mozilla/5.0'})
                    sub_r.raise_for_status()
                    raw = sub_r.text
                except Exception as e:
                    last_err = f'{inst}: sub download {e}'
                    continue
                text = vtt_to_text(raw)
                return jsonify({
                    'video_id': video_id,
                    'source': inst,
                    'lang': chosen.get('code'),
                    'auto_generated': chosen.get('autoGenerated', False),
                    'text': text,
                    'truncated': False,
                    'char_count': len(text),
                    'meta': {
        ...
        # Phase 2: Invidious fallback (другая схема: captions[])
        for inst in cfg.INVIDIOUS_INSTANCES:
            try:
                j = requests.get(f'{inst}/api/v1/videos/{video_id}', timeout=20,
                                 headers={'User-Agent': 'Mozilla/5.0'}).json()
            except Exception as e:
                last_err = f'{inst}: {e}'
                continue
            ...
            chosen = None
            for c in captions:
                if (c.get('languageCode') or c.get('label', '')).lower().startswith(prefer_lang):
                    chosen = c; break
            if not chosen:
                chosen = captions[0]
            try:
            ...
            return jsonify({
                'video_id': video_id,
                'source': inst,
                'lang': chosen.get('languageCode') or chosen.get('label'),
                'auto_generated': True,
                'text': text,
                'truncated': False,
                'char_count': len(text),
                'meta': {
                    'title': j.get('title', ''),
                    'uploader': j.get('author', ''),
                    'duration': j.get('lengthSeconds', 0),
        ...
        return jsonify({
            ...
            'last_err': last_err,
            'hint': 'no auto-subtitles or all mirrors down',
        }), 404
